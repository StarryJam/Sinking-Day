...
public class Grid : MonoBehaviour {
    ...
    public List<Node> moveRangeNodes;
    ...
    private void UpdateMap()
    {
        foreach (var node in gridNodes)
        {
            ...
            node.mapCube.cube.GetComponent<MeshRenderer>().material = node.mapCube.material_Normal;
            ...
            else
            {
                if (isShowPath)
                {
                    if (path.Contains(node))
                    {
                        ...
                        node.mapCube.cube.GetComponent<MeshRenderer>().material = node.mapCube.material_InPath;
                    }
                    else if (moveRangeNodes.Contains(node))
                    {
    ...
    public void GenerateMoveRange(Vector3 startPosition, int range)
    {
        List<Node> tempNodes = new List<Node>();
        Node startNode = GetNodeFromPosition(startPosition);
        moveRangeNodes = new List<Node>();
        moveRangeNodes.Add(startNode);
        for (int i = 0; i < range; i++)
        {
            int count = moveRangeNodes.Count;
            for(int j=0; j < count; j++)
            {
                tempNodes = GetNeiborNodes(moveRangeNodes[j], false);
                foreach(var node in tempNodes)
                {
                    if (!moveRangeNodes.Contains(node) && node.state != Node.NodeState.unwalkable)
                        moveRangeNodes.Add(node);
                }
            }
        }
    }
    ...
    private List<Node> GetNeiborNodes(Node node, bool isDiagonal)
    {
